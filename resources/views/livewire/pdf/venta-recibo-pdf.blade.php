...
<head>
    <meta charset="UTF-8">
    <title>Recibo de Venta - {{ $datosRecibo['nro_recibo'] }}</title>
    <style>
        /* Tipografía y márgenes para hoja tamaño Carta (Comprimido) */
...
<body>
    ...
    <table class="header-table">
        <tr>
            <td width="25%" class="text-left" valign="middle">
                {{-- TRUCO VITAL PARA DOMPDF: public_path() --}}
    ...
    <table class="comprobante-header">
        <tr>
            <td class="text-left"><h2 class="comprobante-title">Comprobante de Venta</h2></td>
    ...
    <table class="items-table">
        ...
        <tbody>
            ...
            <tr>
                <td class="center" style="font-weight: bold; font-size: 13px;">
                    {{ $item['cantidad'] }}
                </td>
                <td class="text-left">
                    <strong style="font-size: 13px;">{{ $item['nombre'] }}</strong><br>
                    <span style="font-size: 10px; color: #666;">Precio Unit: {{ number_format($item['precio'], 2) }} Bs</span>
                </td>
    ...
    <div class="footer">
        ...
        <strong style="color: #111; font-size: 12px;">¡Gracias por su preferencia!</strong>
    </div>
